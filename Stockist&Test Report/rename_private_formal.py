...
# -*- coding: utf-8 -*-
"""
脚本用于重命名 Private Formal 文件夹中的文件
格式：{DN_No}_Private_Formal.pdf
DN No 可能是 C0146, C0152, SS79262, NW00001, SF00001 等格式
"""
...
def extract_dn_no(filename):
    """
    从文件名中提取 DN No
    支持格式：C0146, C0152, SS79262, NW00001, SF00001 等
    ...
    # 例如：C0146, C0152, SS79262, NW00001, SF00001
...
def rename_files(folder_path):
    ...
    for pdf_file in pdf_files:
        ...
        if dn_no:
            new_name = f"{dn_no}_Private_Formal.pdf"
...
if __name__ == "__main__":
    ...
    private_formal_folder = script_dir / "Private Formal"
    
    print("=" * 60)
    print("Private Formal 文件重命名脚本")
    print("=" * 60)
    print(f"目标文件夹：{private_formal_folder}\n")
    
    # 询问用户确认
    response = input("是否继续重命名文件？(y/n): ").strip().lower()
    if response == 'y' or response == 'yes':
        rename_files(private_formal_folder)
    else:
        print("操作已取消")
