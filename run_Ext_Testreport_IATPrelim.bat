@echo off
REM ============================================
REM Run Ext_Testreport_IATPrelim.py Script
REM ============================================
chcp 65001 >nul 2>&1
cd /d "%~dp0"
echo ============================================
echo Running Ext_Testreport_IATPrelim.py
echo ============================================
echo.

REM Try py command first (Python Launcher)
py Ext_Testreport_IATPrelim.py
...
if errorlevel 1 (
    ...
    REM Try python3
    python3 Ext_Testreport_IATPrelim.py 2>nul
    if errorlevel 1 (
        REM Try full Python path
        for /f "tokens=*" %%i in ('where py') do (
            "%%i" Ext_Testreport_IATPrelim.py
            goto :end
        )
