...
class StockistTestDownloader:
            ...
            try:
                ...
                for row in rows:
                    ...
                    if stockist_cert:
                        ...
                        if stockist_cert:
                            batch_results[order_no]['stockist_certs'].add(stockist_cert)
    ...
    def get_all_cert_dn_values(self, order_no: int) -> Tuple[List[str], List[str]]:
        ...
        try:
            ...
            for row in rows:
                ...
                if stockist_cert:
                    stockist_cert = str(stockist_cert).strip()
                    if stockist_cert and stockist_cert not in stockist_certs:
                        stockist_certs.append(stockist_cert)
    ...
    def find_files_by_keywords(self, folder: str, keywords: List[str], search_subfolders: bool = True) -> List[str]:
        ...
        if search_subfolders:
            ...
            # 只打印统计信息
            if found_files:
                print(f"[搜索] {folder}: 扫描 {file_count} 个文件，找到 {len(found_files)} 个匹配的 PDF")
        else:
    ...
    def get_dd_no_by_order(self, order_no: int) -> Optional[str]:
        ...
        try:
            ...
            if row and row['dd_no']:
                dd_no = str(row['dd_no']).strip()
    ...
    def get_dd_no_by_orders_batch(self, order_nos: List[int]) -> Dict[int, str]:
        ...
        for i in range(0, len(order_nos), BATCH_SIZE):
            ...
            try:
                ...
                for row in rows:
                    try:
                        order_no = int(row['bbs_no'])
                        dd_no = str(row['dd_no']).strip() if row['dd_no'] else None
                        if dd_no:
    ...
    def download_by_order_nos_grouped_by_date(self, order_nos: List[int]) -> Tuple[str, int]:
        ...
        # 2. 为每个日期收集文件
        # 结构：{date_str: {stockist_cert: [file_paths]}}
        files_by_date_and_cert = {}  # {date_str: {stockist_cert: [file_paths]}}
        ...
        for date_str in unique_dates:
            ...
            # 按照 stockist_cert 组织文件（同一天的所有stockist放到同一个文件夹并去重）
            files_by_stockist_cert = {}
            ...
            # 将所有文件按照 stockist_cert 分类（去重）
            seen_files = set()  # 用于去重，避免同一文件被添加到多个 stockist_cert
            for file_path in all_files:
                # 如果文件已经处理过，跳过（去重）
            ...
            # 保存该日期的文件组织结构
            files_by_date_and_cert[date_str] = files_by_stockist_cert
        
        if not files_by_date_and_cert:
            raise ValueError(f"所有日期 {unique_dates} 都没有找到相关 PDF 文件")
        
        # 统计各日期的文件数
        date_file_counts = {}
        for date_str, files_by_cert in files_by_date_and_cert.items():
            total_count = sum(len(files) for files in files_by_cert.values())
        ...
        try:
            with zipfile.ZipFile(temp_zip_path, 'w', zipfile.ZIP_STORED) as zipf:
                # 为每个日期和 stockist_cert 组合创建文件夹并添加文件
                for date_str, files_by_stockist_cert in files_by_date_and_cert.items():
                    for stockist_cert, files in files_by_stockist_cert.items():
                        if not files or not stockist_cert:
                            continue
                            ...
                            try:
                                if preserve_folder_structure:
                                    ...
                                    if base_folder_path:
                                        ...
                                        zip_path = f"{date_str}/{stockist_cert}/{source_folder}/{rel_path}"
                                    else:
                                        zip_path = f"{date_str}/{stockist_cert}/{source_folder}/{file_name}"
                                else:
                                    # 其他文件夹（IAT Prelim、Private Formal、Private Prelim、Stockist Cert）：
                                    # 直接放在 日期/stockist_cert 文件夹下，只放文件名
                                    zip_path = f"{date_str}/{stockist_cert}/{file_name}"
                                ...
                                if preserve_folder_structure:
                                    ...
                                    # 其他文件夹直接放在 日期/stockist_cert 文件夹下
                                    zip_path = f"{date_str}/{stockist_cert}/{file_name}"
                                zipf.write(file_path, zip_path)
